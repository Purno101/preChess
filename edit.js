// edit.js — Robust Edit Mode with safe Undo, Reset, and initial-position guard
(() => {
  ...
  function initEditModule() {
    // --- module state ---
    let editMode = false;
    let currentLine = [];        // ✅ raw moves "e2-e4"
    let sanLine = [];            // ✅ human-readable SAN for display
    let editUndoStack = [];
    let moveLog = [];            // { moveStr, from, to, preSnapshot, promotionType }
  ...
  if (!editMode) {
    ...
    editMode = false;
    window.activeMode = null; // free the lock
  ...
  updateButtons();
    ...
    saveBtn.addEventListener('click', () => {
      if (currentLine.length === 0) { alert('No moves to save.'); return; }
      const name = (lineNameInput && lineNameInput.value) ? lineNameInput.value.trim() : '';
      const entry = {
        name: name || `Line ${repertoire.length + 1}`,
        moves: [...currentLine],   // ✅ save raw moves ("e2-e4")
        san: [...sanLine],         // ✅ optional SAN for reference
        created: Date.now()
      };
    ...
    function rebuildCurrentLine() {
      ...
      currentLine = moveLog.map(ctx => ctx.moveStr); // ✅ raw moves
      sanLine = results;                             // ✅ pretty SAN
      updateCurrentLineDisplay();
      updateButtons();
    }
